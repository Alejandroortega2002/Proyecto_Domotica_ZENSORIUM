...
public class Controlador_Registro {
	...
	private void registrarUsuario() throws IOException {
		...
		if (!email.isEmpty() && !password.isEmpty() && !username.isEmpty() && !repetirPass.isEmpty()
			...
			// Mostrar mensaje de error si los campos est�n vac�os.
			Error_Label_Registro.setVisible(true);
			Error_Label_Registro.setText("Hay campos vac�os o no se seleccion� un tipo de cuenta.");
		}
	}

	@FXML
	public void btnRegistrarte(MouseEvent event) throws IOException {
		// Implementa la l�gica para cerrar la aplicaci�n
		registrarUsuario();
	}
	...
	public boolean contrasenaSegura(String contrasena) {
		...
		if (contrasena.length() >= 8) { // una contrase�a tiene m�s de igual o mayor a 8 caracteres.
			...
			for (i = 0; i < contrasena.length(); i++) {
				...
				if (Character.isLetter(l)) {// contiene letras o s�mbolos (?!�@�.,�)
					letraOsimbolo = true;
				}
